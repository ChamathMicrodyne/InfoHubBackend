import PromoCodeItems from "../modal/PromoCodeItems.js";

export async function saveItems(req, res) {
  const lastID = await PromoCodeItems.findOne().sort({ id: -1 });
  const newId = lastID && lastID.id ? lastID.id + 1 : 1;

  const item = new PromoCodeItems({
    id: newId,
    name: req.body.name,
...
export async function getItems(req, res) {
  try {
    const items = await PromoCodeItems.find();
    res.json(items);
  } catch (err) {
    res.json({
...
export async function updateItems(req, res) {
  ...
  try {
    const updatedItem = await PromoCodeItems.findOneAndUpdate(
      { id: itemId },
      updatingData,
      { new: true }
    );
    ...
    const allItems = await PromoCodeItems.find().sort({ id: 1 });
