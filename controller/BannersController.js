import BannersItems from "../modal/BannersItems.js";

export async function saveItems(req, res) {
  const lastID = await BannersItems.findOne().sort({ id: -1 });
  const newId = lastID && lastID.id ? lastID.id + 1 : 1;

  const item = new BannersItems({
    id: newId,
    name: req.body.name,
...
export async function getItems(req, res) {
  try {
    const items = await BannersItems.find();
    res.json(items);
  } catch (err) {
    res.json({
...
export async function updateItems(req, res) {
  ...
  try {
    const updatedItem = await BannersItems.findOneAndUpdate(
      { id: itemId },
      updatingData,
      { new: true }
    );
    ...
    const allItems = await BannersItems.find().sort({ id: 1 });
