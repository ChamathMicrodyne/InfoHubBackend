import PlayerItems from "../modal/PlayerItems.js";

export async function saveItems(req, res) {
  const lastID = await PlayerItems.findOne().sort({ id: -1 });
...
export async function getItems(req, res) {
  try {
    const items = await PlayerItems.find();
    res.json(items);
  } catch (err) {
    res.json({
...
export async function updateItems(req, res) {
  ...
  try {
    const updatedItem = await PlayerItems.findOneAndUpdate(
      { id: itemId },
      updatingData,
      { new: true }
    );
    ...
    const allItems = await PlayerItems.find().sort({ id: 1 });
