import JoiningItems from "../modal/JoiningItems.js";

export async function saveItems(req, res) {
  const lastID = await JoiningItems.findOne().sort({ id: -1 });
  const newId = lastID && lastID.id ? lastID.id + 1 : 1;

  const item = new JoiningItems({
    id: newId,
    name: req.body.name,
...
export async function getItems(req, res) {
  try {
    const items = await JoiningItems.find();
    res.json(items);
  } catch (err) {
    res.json({
