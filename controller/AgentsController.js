import AgentsItems from "../modal/AgentsItems.js";

export async function saveItems(req, res) {
  const lastID = await AgentsItems.findOne().sort({ id: -1 });
  const newId = lastID && lastID.id ? lastID.id + 1 : 1;

  const item = new AgentsItems({
    id: newId,
    name: req.body.name,
...
export async function getItems(req, res) {
  try {
    const items = await AgentsItems.find();
    res.json(items);
  } catch (err) {
    res.json({
...
export async function updateItems(req, res) {
  ...
  try {
    const updatedItem = await AgentsItems.findOneAndUpdate(
      { id: itemId },
      updatingData,
      { new: true }
    );
    ...
    const allItems = await AgentsItems.find().sort({ id: 1 });
