import OffersItems from "../modal/OffersItems.js";

export async function saveItems(req, res) {
  const lastID = await OffersItems.findOne().sort({ id: -1 });
  const newId = lastID && lastID.id ? lastID.id + 1 : 1;

  const item = new OffersItems({
    id: newId,
    name: req.body.name,
...
export async function getItems(req, res) {
  try {
    const items = await OffersItems.find();
    res.json(items);
  } catch (err) {
    res.json({
...
export async function updateItems(req, res) {
  ...
  try {
    const updatedItem = await OffersItems.findOneAndUpdate(
      { id: itemId },
      updatingData,
      { new: true }
    );
    ...
    const allItems = await OffersItems.find().sort({ id: 1 });
